  ...
  actions: {
    ...
    async busquedaPelicula({ commit }, query) {
      try {
        const response = await fetch(
          `${API_BASE_URL}/search/tv?language=es&query=${query}`,
          {
            headers: {
              Authorization:
    ...
    async peliculaGuardada({ commit, state }, pelicula) {
      try {
        ...
        const resultGuardados =
          JSON.parse(localStorage.getItem("resultGuardados")) || [];
        ...
        localStorage.setItem(
          "resultGuardados",
          JSON.stringify(resultGuardados)
        );
    ...
    async eliminarPelicula({ commit, state }, peliculaId) {
      try {
        const resultGuardados =
          JSON.parse(localStorage.getItem("resultGuardados")) || [];
        const updatedpeliculas = resultGuardados.filter(
          (pelicula) => pelicula.id !== peliculaId
        );
        localStorage.setItem(
          "resultGuardados",
          JSON.stringify(updatedpeliculas)
        );

        commit(
          "setResultGuardados",
          state.resultGuardados.filter((pelicula) => pelicula.id !== peliculaId)
        );
      } catch (error) {
